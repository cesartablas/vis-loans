...
The relationship between the Interest Rates and the selected range of the variable being considered is seen in two ways: 1) the corresponding points are highlighted in the cloud with a different color (blue), forming a band or bin with the filtered or selected points, and 2) two boxplots are drawn, one for the whole dataset (golden), and another for the points belonging to the range selected, using the same highlighting color, and making it easy to compare both groups side by side.

I considered the alternative of exploring each variable by animating the transitions between each range of the histogram, saving the user from clicking on the bar each time, but I reasoned that it would be preferable to let the user explore only the bars they are interested in, and in the order they prefer, like: going backwards, and forwards again, and to the beginning again, etc; and also let the user hover on the boxplot to check ranges or whiskers if they wish. An animation would be easier but less engaging for the user.
...
Other feedback were not followed, but were duly considered, mainly:
...
- From a user design perspective when clicking the references, the popup that appears does not appear exactly above the 'references' tab but rather is right aligned to the web page (see screenshot attached)
...
> Currently, the x-axis in the main visualization does not have a purpose, aside from spreading out the points in the visualization. Since the box plots are provided alongside this plot, I don't know if readers will get much more useful information out of the jittered scatterplot than they already get out of the box plots.
